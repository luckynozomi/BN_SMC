...
/*    cout<<"descendants: ";
    set<int>::iterator it;
    for(it = _descendant.begin();it!= _descendant.end();it++)
    {
        cout<< *it <<",";
    }
    cout<<endl;
    cout<<"parents: ";
    for(int m=0; m<_parent.size();m++)
    {
        cout<<_parent[m]<<",";
    }
    cout<<endl;
*/
 ...
 /*   void UpdateScoreOfNeighbors()
    {
        for(unsigned i = 0; i<_neighbors.size();i++)
        {
            _scoreOfNeighbors.push_back(_correspondingConstraint.Get_testScore(_neighbors[i]));
        }
    }
*/
